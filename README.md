# Heart-Attack-Risk-Prediction-Workshop-One
...
- `--host 127.0.0.8`: Позволяет серверу прослушивать запросы извне (актуально при размещении сервиса в локальной сети).
- `--port 8010`: Определяет порт, на котором будет запущен сервер. По умолчанию используется порт 8000.
...
```
uvicorn app.main:app --host 0127.0.0.1 --port 8010
```
